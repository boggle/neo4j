...
trait CodeGenSugar extends MockitoSugar {
  ...
  def executeCompiled(plan: CompiledPlan,
    ...
    try {
      ...
      val transactionalContext = new TransactionalContextWrapperv3_1(Neo4jTransactionalContext.create(graphDb, QuerySourceDescriptor.UNKNOWN, tx, statement, "X", Collections.emptyMap(), locker))
