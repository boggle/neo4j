...
import org.neo4j.kernel.impl.query.{Neo4jTransactionalContext, QueryEngineProvider, QuerySession, QuerySourceDescriptor}
...
trait GraphIcing {
  ...
  implicit class RichGraphDatabaseQueryService(graphService: GraphDatabaseQueryService) {
    ...
    private def createSession(txType: Type): (InternalTransaction, QuerySession) = {
      ...
      val transactionalContext = Neo4jTransactionalContext.create(graphService, QuerySourceDescriptor.UNKNOWN, tx, txBridge.get(), "X", Collections.emptyMap(), locker)
