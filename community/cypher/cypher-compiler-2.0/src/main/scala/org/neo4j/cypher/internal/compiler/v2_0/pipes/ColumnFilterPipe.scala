...
class ColumnFilterPipe(source: Pipe, val returnItems: Seq[ReturnItem])
  ...
  protected def internalCreateResults(input: Iterator[ExecutionContext], state: QueryState) = {
    input.map(ctx => {
      ...
      returnItems.foreach {
        case ReturnItem(Identifier(oldName), newName, _, _) if isNamed(newName) => newMap.put(newName, ctx(oldName))
        case ReturnItem(CachedExpression(oldName, _), newName, _, _)            => newMap.put(newName, ctx(oldName))
        case ReturnItem(_, name, _, _)                                          => newMap.put(name, ctx(name))
      }
