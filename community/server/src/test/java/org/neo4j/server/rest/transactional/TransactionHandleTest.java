...
public class TransactionHandleTest
{
    ...
    public void shouldExecuteStatements() throws Exception
    {
        ...
        when( kernel.create( any( HttpServletRequest.class ), any( GraphDatabaseQueryService.class ),
                any( AccessMode.class ), any( Type.class ), "X", Collections.emptyMap() ) ).thenReturn( querySession );
    ...
    public void shouldHandleExecutionEngineThrowingUndeclaredCheckedExceptions() throws Exception
    {
        // given
        QueryExecutionEngine executionEngine = mock( QueryExecutionEngine.class );
        when( executionEngine.executeQuery( eq( "match (n) return n" ), eq( map() ), any( QuerySession.class ) ) ).thenAnswer(
