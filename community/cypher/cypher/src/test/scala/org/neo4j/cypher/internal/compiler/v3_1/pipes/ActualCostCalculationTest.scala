...
class ActualCostCalculationTest extends CypherFunSuite {
  ...
  private def runSimulation(graph: GraphDatabaseQueryService, pipes: Seq[Pipe]) = {
    ...
    graph.withTx { tx =>
      val transactionalContext = TransactionalContextWrapperv3_1(Neo4jTransactionalContext.create(graph, QuerySourceDescriptor.UNKNOWN, tx, graph.statement, "X", Collections.emptyMap(), new PropertyContainerLocker))
