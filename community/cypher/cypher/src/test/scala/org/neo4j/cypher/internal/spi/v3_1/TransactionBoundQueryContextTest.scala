...
import org.neo4j.kernel.impl.query.{Neo4jTransactionalContext, QuerySourceDescriptor}
...
class TransactionBoundQueryContextTest extends CypherFunSuite {
  ...
  test("should mark transaction successful if successful") {
    ...
    val tc = Neo4jTransactionalContext.create( graph, QuerySourceDescriptor.UNKNOWN, outerTx, statement, "X", Collections.emptyMap(), locker )
  ...
  test("should mark transaction failed if not successful") {
    ...
    val transactionalContext = new TransactionalContextWrapperv3_1(Neo4jTransactionalContext.create(graph, QuerySourceDescriptor.UNKNOWN, outerTx, statement, "X", Collections.emptyMap(), locker))
    val context = new TransactionBoundQueryContext(transactionalContext)(indexSearchMonitor)
    // WHEN
  ...
  test("should return fresh but equal iterators") {
    ...
    val transactionalContext = new TransactionalContextWrapperv3_1(Neo4jTransactionalContext.create(graph, QuerySourceDescriptor.UNKNOWN, outerTx, stmt, "X", Collections.emptyMap(), locker))
    val context = new TransactionBoundQueryContext(transactionalContext)(indexSearchMonitor)
