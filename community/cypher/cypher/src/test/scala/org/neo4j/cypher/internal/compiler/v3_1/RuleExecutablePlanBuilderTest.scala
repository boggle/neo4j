...
class RuleExecutablePlanBuilderTest
  ...
  test("should resolve property keys") {
    ...
    try {
      ...
      val transactionalContext = TransactionalContextWrapperv3_1(Neo4jTransactionalContext.create(graph, QuerySourceDescriptor.UNKNOWN, tx, statement, "X", Collections.emptyMap(), locker))
