...
case class CypherCompiler(parser: CypherParser,
  ...
  def planPreparedQuery(syntacticQuery: PreparedQuerySyntax,
    ...
    val plan = Iterator.continually {
      cacheAccessor.getOrElseUpdate(cache)(semanticQuery.statement, {
        planned = true
        executionPlanBuilder.build(context, semanticQuery, tracer)
      })
    }.flatMap { plan =>
      if (!planned && plan.isStale(context.txIdProvider, context.statistics)) {
        cacheAccessor.remove(cache)(syntacticQuery.statement, syntacticQuery.queryText)
        None
      } else {
        Some(plan)
      }
    }.next()
    (plan, semanticQuery.extractedParams)
  }
